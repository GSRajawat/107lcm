...
if os.path.exists(assigned_file):
    assigned_df = pd.read_csv(assigned_file, dtype={"Roll Number": str})
else:
...
if selected_paper:
    ...
    if st.button("✅ Assign Seats", key="assign_button"):
        ...
        occupied_physical_seat_keys = set(tuple(x) for x in assigned_df[
            (assigned_df["Room Number"] == room) &
            (assigned_df["Date"] == date) &
            (assigned_df["Shift"] == shift)
        ...
        for i in range(1, format_capacity_for_assignment + 1):
            ...
            prospective_seat_key = (room, prospective_seat_string, date, shift)
        ...
        for i, roll in enumerate(students_to_assign):
            ...
            current_assignment_key = (room, seat_num_str, date, shift)
        ...
        # We now simply concatenate. The `assigned_df` will grow with unique physical seat assignments.
        assigned_df = pd.concat([assigned_df, new_assignments_df], ignore_index=True)
        # No drop_duplicates here, as the pre-check and `occupied_physical_seat_keys.add` should handle uniqueness.
